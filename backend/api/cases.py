...
async def create_case(
    ...
    # Every query is gated behind Proceed / Cancel. For RAG the answer is
    # DEFERRED — generated by POST /cases/{id}/answer-rag only if the user
    # proceeds (no wasted LLM call on cancel).
    is_rag = route.strategy == "rag"
    if is_rag or route.strategy == "single":
        is_multi = False
    ...
    if is_rag:
        # No governed scenario — the case just gates the generative answer.
        first_scenario: Optional[str] = None
        interpreted_as = "answer generatively — no governed scenario fits"
        clarifying_question = (
            "No governed scenario fits this question — I'll answer it from the "
            "policy corpus or general knowledge. Proceed?"
        )
        candidates_payload = []  # don't offer weak keyword-match alternatives
    elif is_multi:
        first_scenario = plan.steps[0].scenario_id
    ...
    record = CaseRecord(
        ...
        phase="awaiting_clarification" if (has_candidates or is_multi or is_rag) else "cancelled",
    ...
    return CreateCaseOut(
        ...
        routing=route,
        rag=None,  # RAG answer is deferred until the user proceeds
    )
...
@router.post("/cases/{case_id}/answer-rag", response_model=RagAnswer)
async def answer_rag_case(
    case_id: str, request: Request, user: CurrentUser = Depends(current_user)
) -> RagAnswer:
    """Generate the deferred RAG answer once the user proceeds (Strategy C).

    Separate from the scenario `confirm` path because a RAG case has no
    governed scenario to run — it just retrieves + synthesises over the policy
    corpus. Marks the case complete for the audit trail.
    """
    state: AppState = request.app.state.app_state
    case = _own_case_or_403(state, case_id, user)
    rag = await answer_via_rag(state, case.prompt)
    case.phase = "complete"
